...
class P3ViewController: UIViewController {
    ...
    @IBAction func getImageRGB(_ sender: UITapGestureRecognizer) {
        ...
        self.performSegue(withIdentifier: "toP4", sender: nil)
    }
    
    //受渡し
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        //次の画面を取得（次の画面のオブジェクトを取得する）
        let P4ViewController = segue.destination as! P4ViewController
        //次の画面の変数にこの画面の変数を入れていく
        P4ViewController.passedImage = image
        //RCB値の受け渡し
        P4ViewController.passedRGB =  passedRGB + RGB
    }
